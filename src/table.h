...
typedef struct
{
    ...
    tunit_t tunit; // tunit_t는 일반적인 항공 관제 방식에 따라 5분을 1으로 하며, 소수점 단위를 포함합니다.
                   // 본 프로그램에서는 5분 단위의 슬롯을 채택합니다 (김포공항과 동일한 방식 입니다. ) 
} tnode_t;
...
// tunit_t에서 mhd_t를 생성하여 반환합니다. 값을 직접 반환하므로 메모리 할당을 하지 않습니다. 
mhd_t get_mhd(tunit_t t);
// 예를 들어 3.5인 경우는 4로 올립니다. 이유는 슬롯이 5분 단위이기 때문이고 인덱스는 정수형이어야 하기 때문입니다. 
int ceil_tunit(tunit_t t);
// 특정 시간대에 해당 도시로 가는지 찾습니다. 선형탐색이라 시간이 매우 많이 걸립니다. 
