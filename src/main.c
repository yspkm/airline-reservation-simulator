    ...
    while (true)
    {
        ...
        case INSERTTION:
            if (cnt_reservatinos >= MAX_RESERVATION) {
                printf("full of reservatoins: %d >= %d", cnt_reservatinos, MAX_RESERVATION);
                break;
            }
            ...
            if (UNFOUND == find_path(gptr, iptr->src - 'a', iptr->dst - 'a', iptr->day, optr))
            {
        ...
        case SEARCH_PATH:
            ...
            if (UNFOUND == find_path(gptr, iptr->src - 'a', iptr->dst - 'a', iptr->day, &path_buf))
            {
                printf("path not found\n");
                break;
            }
